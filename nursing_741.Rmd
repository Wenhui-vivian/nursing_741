...
From viewing thism perhaps variables like Arrest, Domestic, FBI Code, District, and Ward could be good summary variables. We don't really have any kind of measured data here. 
...
So let's plot a bar chart that shows is the total number of calls to the police for each month and what portion of those calls each monht resulted in an arrest.
...
```{r etmap13,cache=TRUE,eval=FALSE}
library(ggmap)
...
We can use some predefined terms such as "..level.." to get counts of the lat lons in an area which will then be reflected in "fill". This plot winds up being very cool and informative. We can see that the Nartotics offenses are happening on the East Side and near the University Village and Little Italy. 

```{r getmap13,cache=TRUE,eval=FALSE}
...
**Leaflet** is a javascript library that simplifies the creation of maps and placement of markers there upon. R has a companion package called "leaflet" that you can use to help create some really nice maps. All that is required is some latitutde and longtiude information which we already have since we are working with the Chicago data. 

Let's continue with the Narcotics data and see where these are occurring. I think you will like the output that leafelt provides because it is interactive and you can zoom in and out of the data
